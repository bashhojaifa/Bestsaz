...
class SuggestedProducts extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(ApiURL.SuggestedProducts(subcategory))
      .then((response) => {
  ...
  render() {
    ...
    var data = SuggestedProducts.map((MyData, i) => {
      if (MyData.discount_price == "NA") {
        return (
          <Col className="mb-2" xl={3} lg={3} md={3} sm={6} xs={12}>
            <Link to={"/product-details/" + MyData.product_code}>
              <Card className="image-box  w-100 card">
                <img className="" src={MyData.image} />
                <Card.Body>
        ...
        return (
          <Col className="mb-2" xl={2} lg={2} md={4} sm={3} xs={12}>
            <Link to={"/product-details/" + MyData.product_code}>
              <Card className="image-box  w-100 card">
                <img className="" src={MyData.image} />
                <Card.Body>
                  ...
                  <p className="product-price-on-card">
                    <strike>Before: {MyData.price}</strike>
                  </p>
                  <p className="product-price-on-card">
                    After: {MyData.discount_price}
                  </p>
                </Card.Body>
              </Card>
            </Link>
